...
def move_to_review(request, article_id, should_redirect=True):
    ...
    if article.editorassignment_set.all().count() > 0:
        ...
        article.save()
        review_round, created = models.ReviewRound.objects.get_or_create(
        ...
        if not created:
            messages.add_message(
                request,
                messages.WARNING,
                "A default review round already exists for this article.",
            )
        ...
        messages.add_message(
            request,
            messages.INFO,
            "You must assign an editor before moving into reivew.",
        )
    ...
    if should_redirect:
        if request.GET.get("return", None):
            ...
            return redirect(
                "{0}?modal_id={1}".format(reverse("kanban_home"), article_id)
            )
...
def in_review(request, article_id):
    ...
    if request.POST:
        if (
            "move_to_review" in request.POST
            and article.stage == submission_models.STAGE_UNASSIGNED
        ):
        ...
        if "new_review_round" in request.POST:
            ...
            for (
                assignment
            ) in article.current_review_round_object().reviewassignment_set.all():
                if not assignment.date_complete:
                    ...
                    messages.add_message(
                        request,
                        messages.INFO,
                        "Assignment {0} closed.".format(assignment.id),
                    )

                kwargs = {"review_assignment": assignment, "request": request}
                event_logic.Events.raise_event(
                    event_logic.Events.ON_REVIEW_CLOSED,
            ...
            models.ReviewRound.objects.create(
                article=article, round_number=new_round_number
            )
...
def send_review_reminder(request, article_id, review_id, reminder_type):
    ...
    email_content = logic.get_reminder_content(
        reminder_type, article, review_assignment, request
    )

    form_initials = {"body": email_content, "subject": "Review Request Reminder"}
    ...
    if request.POST:
        if "delete" in request.POST:
            ...
            messages.add_message(
                request,
                messages.INFO,
                "Round {0} deleted.".format(review_round.round_number),
            )
            return redirect(
...
def add_files(request, article_id, round_id):
    ...
    if request.POST:
        if "upload" in request.POST:
            ...
            if review_files:
                for review_file in review_files:
                    new_file_obj = files.save_file_to_article(
                        review_file, article, request.user, "Review File"
                    )
            ...
            return redirect(
                reverse(
                    "review_add_files",
                    kwargs={"article_id": article.pk, "round_id": review_round.pk},
                )
            )
        ...
        for file in request.POST.getlist("file"):
            ...
            messages.add_message(
                request, messages.INFO, "File {0} added.".format(file.label)
            )
...
def remove_file(request, article_id, round_id, file_id):
    ...
    if review_round.round_number == article.current_review_round():
        ...
        messages.add_message(
            request, messages.INFO, "File {0} removed.".format(file.label)
        )
    else:
        messages.add_message(
            request,
...
def accept_review_request(request, assignment_id):
    ...
    kwargs = {"review_assignment": assignment, "request": request, "accepted": True}
    event_logic.Events.raise_event(
...
def decline_review_request(request, assignment_id):
    ...
    kwargs = {"review_assignment": assignment, "request": request, "accepted": False}
    event_logic.Events.raise_event(
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.INFO,
                "Thanks for suggesting reviewers for this article.",
            )
...
def review_requests(request):
    ...
    completed_requests = models.ReviewAssignment.objects.filter(
        Q(is_complete=True) & Q(reviewer=request.user), article__journal=request.journal
    ).select_related("article")
...
def do_review(request, assignment_id):
    ...
    if request.POST:
        if request.FILES:
            assignment = upload_review_file(request, assignment_id=assignment_id)
        ...
        if "assign" in request.POST:
            ...
            if valid:
                ...
                return redirect(
                    ...
                    + "?"
                    + parse.urlencode(
                        {"user": new_reviewer_form.data["email"], "id": str(acc.pk)}
                    ),
                )
            else:
...
def edit_review(request, article_id, review_id):
    ...
    if review.date_complete:
        messages.add_message(
            request,
            messages.WARNING,
            "You cannot edit a review that is already complete.",
        )
        return redirect(reverse("review_in_review", kwargs={"article_id": article.pk}))
    ...
    if request.POST:
        form = forms.EditReviewAssignmentForm(
            request.POST, instance=review, journal=request.journal
        )
...
def delete_review(request, article_id, review_id):
    ...
    if review.date_complete:
        messages.add_message(
            request,
            messages.WARNING,
            "You cannot delete a review that is already complete.",
        )
        return redirect(reverse("review_in_review", kwargs={"article_id": article.pk}))

    if request.POST and "delete" in request.POST:
        user_message = request.POST.get(
            "delete_rationale", "No message supplied by user."
        )
        description = (
            "Review {0} for article {1} has been deleted by {2}. \n\n{3}".format(
                review.pk,
                article.title,
...
def review_decision(request, article_id, decision):
    ...
    if (article.date_accepted or article.date_declined) and decision != "undecline":
        messages.add_message(
            request,
            messages.WARNING,
            _("This article has already been accepted or declined."),
        )
        return redirect(reverse("review_in_review", kwargs={"article_id": article.pk}))
...
def rate_reviewer(request, article_id, review_id):
    ...
    if not review.is_complete:
        messages.add_message(
            request,
            messages.INFO,
            "You cannot rate a reviewer until their review is complete."
            "You should withdraw this review if you want to rate the reviewer"
            "before they are finished.",
        )
        return redirect(
            reverse("review_in_review", kwargs={"article_id": review.article.id})
        )
    ...
    if request.POST:
        ...
        if review.review_rating:
            ...
            messages.add_message(
                request,
                messages.INFO,
                "{0}'s rating updated to {1}".format(
                    review.reviewer.full_name(), rating_int
                ),
            )
        else:
            messages.add_message(
                request,
                messages.INFO,
                "{0} assigned a rating of {1}".format(
                    review.reviewer.full_name(), rating_int
                ),
            )
            models.ReviewerRating.objects.create(
                assignment=review, rating=rating_int, rater=request.user
            )
...
def edit_revision_request(request, article_id, revision_id):
    ...
    if revision_request.date_completed:
        messages.add_message(
            request,
            messages.WARNING,
            "You cannot edit a revision request that is complete.",
        )
        return redirect(reverse("review_in_review", kwargs={"article_id": article_id}))
    ...
    if request.POST:
        ...
        if "delete_revision" in request.POST:
            ...
            util_models.LogEntry.add_entry(
                "deletion",
                "{0} deleted a revision request with reason:\n\n{1}".format(
                    request.user.full_name(), rationale
                ),
                level="Info",
                actor=request.user,
        ...
        if "mark_as_complete" in request.POST:
            util_models.LogEntry.add_entry(
                "update",
                "{0} marked revision {1} as complete".format(
                    request.user.full_name(), revision_request.id
                ),
                level="Info",
                actor=request.user,
                target=revision_request.article,
            ...
            messages.add_message(
                request, messages.INFO, "Revision request marked as complete."
            )
    ...
    if request.POST:
        ...
        if "delete" in request.POST:
            ...
            if form.is_valid() and form.is_confirmed():
                ...
                event_logic.Events.raise_event(
                    event_logic.Events.ON_REVISIONS_COMPLETE, **kwargs
                )
                messages.add_message(
                    request,
                    messages.SUCCESS,
...
def replace_file(request, article_id, revision_id, file_id):
    ...
    if request.POST and request.FILES:
        if "replacement" in request.POST:
            ...
            logic.log_revision_event(
                "File {0} ({1}) replaced with {2} ({3})".format(
                    file.label,
...
def reviewer_article_file(request, assignment_id, file_id):
    ...
    return files.serve_file(request, file_object, article_object, hide_name=True)
    ...
    if request.POST:
        if "delete" in request.POST:
            ...
            draft = get_object_or_404(
                models.DecisionDraft, pk=delete_id, article=article
            )
            draft.delete()
            return redirect(
                reverse(
...
def manage_draft(request, article_id, draft_id):
    ...
    messages.add_message(
        request, messages.INFO, "Draft {}".format(draft.editor_decision)
    )
    ...
    if request.POST:
        if "delete" in request.POST:
            ...
            element_to_delete = get_object_or_404(
                models.ReviewFormElement, pk=delete_id
            )
            element_to_delete.delete()
            return redirect(
                reverse("edit_review_form", kwargs={"form_id": edit_form.pk})
            )
...
def decision_helper(request, article_id):
    ...
    if request.POST:
        ...
        if "review_file_visible" in request.POST:
            ...
            logic.handle_review_file_switch(
                review, request.POST.get("review_file_visible")
            )
            messages.add_message(
                request, messages.SUCCESS, "Review File visibility updated."
            )
...
def upload_reviewers_from_csv(request, article_id):
    ...
    if request.POST and request.FILES:
        ...
        if form.is_valid():
            reviewers, import_error = logic.process_reviewer_csv(
                path, request, article, form
            )
...
def editor_share_reviews(request, article_id):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, "Reviews shared with reviewers."
            )
            return redirect(
                reverse(
                    "decision_helper",
...
def reviewer_shared_review_download(request, article_id, review_id):
    ...
    if request.journal.get_setting(
        ...
        current_round_reviewers = [
            review.reviewer
            for review in models.ReviewAssignment.objects.filter(
                article=article,
